...
public:
    ...
    void read(const FileNode& node)                          //Read serialization for this class
    {
        ...
        node["Fix_K4"] >> fixK4;
        node["Fix_K5"] >> fixK5;
        node["Camera_Exposure"] >> exposure;

        if (!node["Image_Width"].isNone() && !node["Image_Height"].isNone())
        {
    ...
    void validate()
    {
        ...
        if (goodInput)
        {
            ...
            {
                inputCapture.set(CAP_PROP_FRAME_WIDTH, imageSize.width);
                inputCapture.set(CAP_PROP_FRAME_HEIGHT, imageSize.height);
            }
            inputCapture.set(cv::CAP_PROP_AUTO_EXPOSURE, 0);
            inputCapture.set(CAP_PROP_EXPOSURE, static_cast<double>(exposure));
        }
